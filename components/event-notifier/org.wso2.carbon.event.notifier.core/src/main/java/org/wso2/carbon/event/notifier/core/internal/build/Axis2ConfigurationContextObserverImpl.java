...
* under the License.
*/
package org.wso2.carbon.event.notifier.core.internal.build;
...
public class Axis2ConfigurationContextObserverImpl
    ...
    public void createdConfigurationContext(ConfigurationContext configurationContext) {
        ...
        System.out.println("--------------- A new Axis2 Configuration context is created for : " +
                tenantDomain);
        log.info("Loading Event Notifiers Specific to tenant when the tenant logged in");
        try {
            PrivilegedCarbonContext.startTenantFlow();
            PrivilegedCarbonContext.getThreadLocalCarbonContext().setTenantId(tenantId);
            PrivilegedCarbonContext.getThreadLocalCarbonContext().getTenantDomain(true);
        } catch (Exception e) {
            log.error("Unable to load Event Notifiers ", e);
        } finally {
