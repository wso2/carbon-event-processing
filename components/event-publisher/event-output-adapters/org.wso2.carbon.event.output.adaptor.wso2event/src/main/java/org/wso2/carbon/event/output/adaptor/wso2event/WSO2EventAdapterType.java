...
public final class WSO2EventAdapterType extends AbstractOutputEventAdapter {

    private static final Log log = LogFactory.getLog(WSO2EventAdapterType.class);
    private static WSO2EventAdapterType wso2EventAdaptor = new WSO2EventAdapterType();
    private ResourceBundle resourceBundle;
    private ConcurrentHashMap<Integer, ConcurrentHashMap<OutputAdaptorConfiguration, LoadBalancingDataPublisher>> dataPublisherMap = new ConcurrentHashMap<Integer, ConcurrentHashMap<OutputAdaptorConfiguration, LoadBalancingDataPublisher>>();
    private Agent agent;

    private WSO2EventAdapterType() {
     ...
     */
    public static WSO2EventAdapterType getInstance() {
    ...
    public void publish(
        ...
        ConcurrentHashMap<OutputAdaptorConfiguration, LoadBalancingDataPublisher> dataPublishers = dataPublisherMap.get(tenantId);
        if (dataPublishers == null) {
            dataPublishers = new ConcurrentHashMap<OutputAdaptorConfiguration, LoadBalancingDataPublisher>();
            dataPublisherMap.putIfAbsent(tenantId, dataPublishers);
            dataPublishers = dataPublisherMap.get(tenantId);
        }
        LoadBalancingDataPublisher dataPublisher = dataPublishers.get(outputAdaptorConfiguration);
        if (dataPublisher == null) {
            synchronized (this) {
                dataPublisher = dataPublishers.get(outputAdaptorConfiguration);
                if (dataPublisher == null) {
                    dataPublisher = createDataPublisher(outputAdaptorConfiguration);
                    dataPublishers.putIfAbsent(outputAdaptorConfiguration, dataPublisher);
                }
            }
        }
        ...
        try {
            ...
            if (!dataPublisher.isStreamDefinitionAdded(streamDefinition)) {
                ...
                publishEvent(outputAdaptorConfiguration, dataPublisher, event, streamDefinition);
            } else {
                //Sending Events
                publishEvent(outputAdaptorConfiguration, dataPublisher, event, streamDefinition);
            }
    ...
    private LoadBalancingDataPublisher createDataPublisher(
        ...
        Map<String, String> adaptorOutputProperties = outputAdaptorConfiguration.getEndpointAdaptorProperties();
        ...
        try {
            ...
            throw new EndpointAdaptorProcessingException(
                    "Cannot publish data via DataPublisher for the adaptor configuration:" +
                            outputAdaptorConfiguration.getAdaptorName() + " for the  event " + event, ex);
        }
    ...
    public void testConnection(
            OutputAdaptorConfiguration outputAdaptorConfiguration, int tenantId) {
        try {
            DataPublisher dataPublisher = new DataPublisher(outputAdaptorConfiguration.getEndpointAdaptorProperties().get(WSO2EventAdaptorConstants.ADAPTOR_CONF_WSO2EVENT_PROP_RECEIVER_URL), outputAdaptorConfiguration.getEndpointAdaptorProperties().get(WSO2EventAdaptorConstants.ADAPTOR_CONF_WSO2EVENT_PROP_USER_NAME), outputAdaptorConfiguration.getEndpointAdaptorProperties().get(WSO2EventAdaptorConstants.ADAPTOR_CONF_WSO2EVENT_PROP_PASSWORD));
    ...
    public void removeConnectionInfo(
            OutputAdaptorConfiguration outputAdaptorConfiguration, int tenantId) {
        ConcurrentHashMap<OutputAdaptorConfiguration, LoadBalancingDataPublisher> tenantSpecificAdaptorMap = dataPublisherMap.get(tenantId);
        if (tenantSpecificAdaptorMap != null) {
            LoadBalancingDataPublisher loadBalancingDataPublisher = tenantSpecificAdaptorMap.remove(outputAdaptorConfiguration);
