...
public final class CassandraEventAdapterType extends AbstractOutputEventAdapter {

    private static final Log log = LogFactory.getLog(CassandraEventAdapterType.class);
    private StringSerializer sser = new StringSerializer();
    private static CassandraEventAdapterType cassandraEventAdaptor = new CassandraEventAdapterType();
    private ResourceBundle resourceBundle;
    private ConcurrentHashMap<Integer, ConcurrentHashMap<OutputAdaptorConfiguration, EventAdaptorInfo>> tenantedCassandraClusterCache = new ConcurrentHashMap<Integer, ConcurrentHashMap<OutputAdaptorConfiguration, EventAdaptorInfo>>();

    private CassandraEventAdapterType() {
     ...
     */
    public static CassandraEventAdapterType getInstance() {
    ...
    public void publish(
        ...
        ConcurrentHashMap<OutputAdaptorConfiguration, EventAdaptorInfo> cassandraClusterCache = null;
        if (message instanceof Map) {
            try {

                cassandraClusterCache = tenantedCassandraClusterCache.get(tenantId);
                if (null == cassandraClusterCache) {
                    cassandraClusterCache = new ConcurrentHashMap<OutputAdaptorConfiguration, EventAdaptorInfo>();
                    if (null != tenantedCassandraClusterCache.putIfAbsent(tenantId, cassandraClusterCache)) {
                        cassandraClusterCache = tenantedCassandraClusterCache.get(tenantId);
                    }
                }

                EventAdaptorInfo eventAdaptorInfo = cassandraClusterCache.get(outputAdaptorConfiguration);
                if (null == eventAdaptorInfo) {
                    Map<String, String> properties = outputAdaptorConfiguration.getEndpointAdaptorProperties();
                    ...
                    if (null != cassandraClusterCache.putIfAbsent(outputAdaptorConfiguration, eventAdaptorInfo)) {
                        eventAdaptorInfo = cassandraClusterCache.get(outputAdaptorConfiguration);
                    } else {
                        log.info("Initiated Cassandra Writer " + outputAdaptorConfiguration.getAdaptorName());
                    }
                }


                String keySpaceName = outputAdaptorConfiguration.getEndpointAdaptorProperties().get(CassandraEventAdaptorConstants.ADAPTOR_CASSANDRA_KEY_SPACE_NAME);
                String columnFamilyName = outputAdaptorConfiguration.getEndpointAdaptorProperties().get(CassandraEventAdaptorConstants.ADAPTOR_CASSANDRA_COLUMN_FAMILY_NAME);
                MessageInfo messageInfo = eventAdaptorInfo.getMessageInfoMap().get(outputAdaptorConfiguration);
                if (null == messageInfo) {
                    Keyspace keyspace = HFactory.createKeyspace(keySpaceName, eventAdaptorInfo.getCluster());
                    messageInfo = new MessageInfo(keyspace);
                    if (null != eventAdaptorInfo.getMessageInfoMap().putIfAbsent(outputAdaptorConfiguration, messageInfo)) {
                        messageInfo = eventAdaptorInfo.getMessageInfoMap().get(outputAdaptorConfiguration);
                    }
                }
                ...
                if (cassandraClusterCache != null) {
                    cassandraClusterCache.remove(outputAdaptorConfiguration);
                }
    ...
    public void testConnection(
        ...
        String cassandraHosts = outputAdaptorConfiguration.getEndpointAdaptorProperties().get(CassandraEventAdaptorConstants.ADAPTOR_CASSANDRA_HOSTNAME) + ":" + outputAdaptorConfiguration.getEndpointAdaptorProperties().get(CassandraEventAdaptorConstants.ADAPTOR_CASSANDRA_PORT);

        Map<String, String> credentials = new HashMap<String, String>();
        credentials.put("username", outputAdaptorConfiguration.getEndpointAdaptorProperties().get(CassandraEventAdaptorConstants.ADAPTOR_CASSANDRA_USER_NAME));
        credentials.put("password", outputAdaptorConfiguration.getEndpointAdaptorProperties().get(CassandraEventAdaptorConstants.ADAPTOR_CASSANDRA_PASSWORD));
        ...
        if (knownPools == null || knownPools.size() == 0) {
            throw new EndpointAdaptorProcessingException("Couldn't connect to Cassandra cluster");
        }
    }
    ...
    public void removeConnectionInfo(
            OutputAdaptorConfiguration outputAdaptorConfiguration, int tenantId) {
        ConcurrentHashMap<OutputAdaptorConfiguration, EventAdaptorInfo> cassandraClusterCache = tenantedCassandraClusterCache.get(tenantId);
        if (cassandraClusterCache != null) {
            cassandraClusterCache.remove(outputAdaptorConfiguration);
        }
    }
    ...
    class EventAdaptorInfo {
        ...
        private ConcurrentHashMap<OutputAdaptorConfiguration, MessageInfo> messageInfoMap = new ConcurrentHashMap<OutputAdaptorConfiguration, MessageInfo>();
        ...
        public ConcurrentHashMap<OutputAdaptorConfiguration, MessageInfo> getMessageInfoMap() {
