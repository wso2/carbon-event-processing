...
public class WSO2EventOutputMapper implements OutputMapper {
    ...
    public WSO2EventOutputMapper(EventNotifierConfiguration eventNotifierConfiguration,
        ...
        String outputStreamName = eventNotifierConfiguration.getEndpointAdaptorConfiguration().getOutputAdaptorProperties().get(EventNotifierConstants.EF_ELE_PROPERTY_STREAM_NAME);
        String outputStreamVersion = eventNotifierConfiguration.getEndpointAdaptorConfiguration().getOutputAdaptorProperties().get(EventNotifierConstants.EF_ATTR_VERSION);

        try {
            wso2EventOutputMapping = (WSO2EventOutputMapping) eventNotifierConfiguration.getOutputMapping();
            ...
            throw new EventNotifierConfigurationException("Error while creating output stream definition : " + outputStreamName + ":" + outputStreamVersion, e);
        }
    ...
    private void validateStreamDefinitionWithOutputProperties()
        ...
        for (; metaWSO2EventOutputPropertyConfigurationIterator.hasNext(); ) {
            ...
            if (!propertyPositionMap.containsKey(wso2EventOutputProperty.getValueOf())) {
                throw new EventNotifierStreamValidationException("Property " + wso2EventOutputProperty.getValueOf() + " is not in the input stream definition. ", inputStreamDefinition.getStreamId());
            }
        }
