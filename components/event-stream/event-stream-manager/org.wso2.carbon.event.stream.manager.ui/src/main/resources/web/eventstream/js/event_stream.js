...
function addEventStream(form, option, eventStreamId) {
	...
	var eventStreamName = document.getElementById("eventStreamNameId").value
			.trim();
	var eventStreamVersion = document.getElementById("eventStreamVersionId").value
			.trim();

	var eventStreamDescription = document
			.getElementById("eventStreamDescription").value.trim();
	var eventStreamNickName = document.getElementById("eventStreamNickName").value
			.trim();
					...
					'../eventstream/add_event_stream_ajaxprocessor.jsp',
					{
						...
						onSuccess : function(event) {
							if ("true" == event.responseText.trim()) {
								CARBON
										.showInfoDialog(
												"Stream definition added successfully!!",
												function() {
													CARBON
															.customConfirmDialogBox(
																	"The defined event stream can be populated with in flow of events using an event builder.\nDo you want to create an event builder now? ",
																	"Default WSO2Event Builder",
																	"Custom Event Builder",
																	function(
																			option) {
																		if (option == "custom") {
																			new Ajax.Request(
																					'../eventbuilder/popup_create_event_builder_ajaxprocessor.jsp',
																					{
																						method : 'POST',
																						asynchronous : false,
																						parameters : {
																			...
																			new Ajax.Request(
																					'../eventbuilder/add_default_event_builder_ajaxprocessor.jsp',
																					{
																						method : 'POST',
																						asynchronous : false,
																						parameters : {
																						...
																						onSuccess : function(
																							...
																							if ("true" == response.responseText
																									.trim()) {
																								CARBON
																										.showInfoDialog("Default Event Builder added successfully!!");
																								form
																										.submit();
																							} else {
																								CARBON
																										.showErrorDialog("Failed to add event builder, Exception: "
								...
								CARBON
										.showErrorDialog("Failed to add event stream, Exception: "
							...
							function() {
								new Ajax.Request(
										'../eventstream/edit_event_stream_ajaxprocessor.jsp',
										{
											...
											onSuccess : function(event) {
												if ("true" == event.responseText
														.trim()) {
													form.submit();
												} else {
													CARBON
															.showErrorDialog("Failed to edit event stream, Exception: "
																	+ event.responseText
																			.trim());
												}
											}
...
function addStreamAttribute(dataType) {
	var attributeName = document.getElementById("output" + dataType
			+ "DataPropName");
	var attributeType = document.getElementById("output" + dataType
			+ "DataPropType");
	var streamAttributeTable = document.getElementById("output" + dataType
			+ "DataTable");
	var noStreamAttributesDiv = document.getElementById("noOutput" + dataType
			+ "Data");
	...
	var newTableRow = streamAttributeTable
			.insertRow(streamAttributeTable.rows.length);
	...
	newCel3.innerHTML = ' <a class="icon-link" style="background-image:url(../admin/images/delete.gif)" onclick="removeStreamAttribute(this,\''
			+ dataType + '\')">Delete</a>';
...
CARBON.customConfirmDialogBox = function(message, option1, option2, callback,
		closeCallback) {
				...
				.dialog(
						{
							...
							buttons : {
								"OK" : function() {
									var value = jQuery(
											'input[name=dialogRadio]:checked')
											.val();
									jQuery(this).dialog("destroy").remove();
									jQuery("#dcontainer").empty();
									if (callback
...
function convertEventStreamInfoDtoToString() {
	...
	new Ajax.Request(
			'../eventstream/transform_to_string_ajaxprocessor.jsp',
			{
				method : 'POST',
				asynchronous : false,
                dataType : "text",
...
function convertStringToEventStreamInfoDto() {
	...
	new Ajax.Request(
			'../eventstream/transform_to_dto_ajaxprocessor.jsp',
			{
				method : 'POST',
				asynchronous : false,
				dataType : "text",
				...
				onSuccess : function(data) {
                    ...
                    if(eventStreamDefinitionDtoJSON.success.localeCompare("fail")==0){
                        ...
                        if (0 == eventStreamDefinitionDtoJSON.message.metaAttributes.length) {
                            var streamAttributeTable = document
                                .getElementById("outputMetaDataTable");
                        ...
                        if (0 == eventStreamDefinitionDtoJSON.message.correlationAttributes.length) {
                            ...
                            for (i = 0; i < eventStreamDefinitionDtoJSON.message.correlationAttributes.length; i++) {
                                addStreamAttribute2(
                                    "Correlation",
                                    eventStreamDefinitionDtoJSON.message.correlationAttributes[i].attributeName,
                                    eventStreamDefinitionDtoJSON.message.correlationAttributes[i].attributeType);
                            }
                        ...
                        if (0 == eventStreamDefinitionDtoJSON.message.payloadAttributes.length) {
                            ...
                            for (i = 0; i < eventStreamDefinitionDtoJSON.message.payloadAttributes.length; i++) {
                                addStreamAttribute2("Payload",
                                    eventStreamDefinitionDtoJSON.message.payloadAttributes[i].attributeName,
                                    eventStreamDefinitionDtoJSON.message.payloadAttributes[i].attributeType);
                            }
...
function addEventStreamByString(form) {
    ...
    new Ajax.Request(
        '../eventstream/add_event_stream_by_string_ajaxprocessor.jsp',
        {
            method : 'POST',
            asynchronous : false,
            parameters : {
