...
public final class CassandraEventAdaptorType extends AbstractOutputEventAdaptor {
    ...
    private ConcurrentHashMap<Integer, ConcurrentHashMap<EndpointAdaptorConfiguration, EventAdaptorInfo>> tenantedCassandraClusterCache = new ConcurrentHashMap<Integer, ConcurrentHashMap<EndpointAdaptorConfiguration, EventAdaptorInfo>>();
    ...
    public void publish(
        ...
        ConcurrentHashMap<EndpointAdaptorConfiguration, EventAdaptorInfo> cassandraClusterCache = null;
        if (message instanceof Map) {
            try {

                cassandraClusterCache = tenantedCassandraClusterCache.get(tenantId);
                if (null == cassandraClusterCache) {
                    cassandraClusterCache = new ConcurrentHashMap<EndpointAdaptorConfiguration, EventAdaptorInfo>();
                    if (null != tenantedCassandraClusterCache.putIfAbsent(tenantId, cassandraClusterCache)) {
                        cassandraClusterCache = tenantedCassandraClusterCache.get(tenantId);
                    }
                }

                EventAdaptorInfo eventAdaptorInfo = cassandraClusterCache.get(endpointAdaptorConfiguration);
                if (null == eventAdaptorInfo) {
                    Map<String, String> properties = endpointAdaptorConfiguration.getOutputAdaptorProperties();
                    ...
                    if (null != cassandraClusterCache.putIfAbsent(endpointAdaptorConfiguration, eventAdaptorInfo)) {
                        eventAdaptorInfo = cassandraClusterCache.get(endpointAdaptorConfiguration);
                    } else {
                        log.info("Initiated Cassandra Writer " + endpointAdaptorConfiguration.getEndpointAdaptorName());
                    }
                }


                String keySpaceName = endpointAdaptorConfiguration.getOutputAdaptorProperties().get(CassandraEventAdaptorConstants.ADAPTOR_CASSANDRA_KEY_SPACE_NAME);
                String columnFamilyName = endpointAdaptorConfiguration.getOutputAdaptorProperties().get(CassandraEventAdaptorConstants.ADAPTOR_CASSANDRA_COLUMN_FAMILY_NAME);
                MessageInfo messageInfo = eventAdaptorInfo.getMessageInfoMap().get(endpointAdaptorConfiguration);
                if (null == messageInfo) {
                    Keyspace keyspace = HFactory.createKeyspace(keySpaceName, eventAdaptorInfo.getCluster());
                    messageInfo = new MessageInfo(keyspace);
                    if (null != eventAdaptorInfo.getMessageInfoMap().putIfAbsent(endpointAdaptorConfiguration, messageInfo)) {
                        messageInfo = eventAdaptorInfo.getMessageInfoMap().get(endpointAdaptorConfiguration);
                    }
                }
                ...
                if (cassandraClusterCache != null) {
                    cassandraClusterCache.remove(endpointAdaptorConfiguration);
                }
    ...
    public void testConnection(
        ...
        String cassandraHosts = endpointAdaptorConfiguration.getOutputAdaptorProperties().get(CassandraEventAdaptorConstants.ADAPTOR_CASSANDRA_HOSTNAME) + ":" + endpointAdaptorConfiguration.getOutputAdaptorProperties().get(CassandraEventAdaptorConstants.ADAPTOR_CASSANDRA_PORT);

        Map<String, String> credentials = new HashMap<String, String>();
        credentials.put("username", endpointAdaptorConfiguration.getOutputAdaptorProperties().get(CassandraEventAdaptorConstants.ADAPTOR_CASSANDRA_USER_NAME));
        credentials.put("password", endpointAdaptorConfiguration.getOutputAdaptorProperties().get(CassandraEventAdaptorConstants.ADAPTOR_CASSANDRA_PASSWORD));
    ...
    public void removeConnectionInfo(
            EndpointAdaptorConfiguration endpointAdaptorConfiguration, int tenantId) {
        ConcurrentHashMap<EndpointAdaptorConfiguration, EventAdaptorInfo> cassandraClusterCache = tenantedCassandraClusterCache.get(tenantId);
        if (cassandraClusterCache != null) {
            cassandraClusterCache.remove(endpointAdaptorConfiguration);
        }
    }
    ...
    class EventAdaptorInfo {
        ...
        private ConcurrentHashMap<EndpointAdaptorConfiguration, MessageInfo> messageInfoMap = new ConcurrentHashMap<EndpointAdaptorConfiguration, MessageInfo>();
        ...
        public ConcurrentHashMap<EndpointAdaptorConfiguration, MessageInfo> getMessageInfoMap() {
