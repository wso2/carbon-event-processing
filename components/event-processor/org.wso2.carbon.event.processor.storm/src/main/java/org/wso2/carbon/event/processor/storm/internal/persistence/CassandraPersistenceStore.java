...
* under the License.
*/
package org.wso2.carbon.event.processor.storm.internal.persistence;
...
public class CassandraPersistenceStore implements PersistenceStore {
    ...
    public void save(String queryPlanIdentifier, String revision, byte[] bytes) {
        ...
        mutator.insert(queryPlanIdentifier, new StringBuilder().append(COLUMN_FAMILY_NAME).append("_").append(tenantId).toString(), HFactory.createColumn(revision, bytes, sser, bser));
        mutator.execute();
    }
    ...
    public byte[] load(String queryPlanIdentifier, String revision) {
        ...
        q.setColumnFamily(COLUMN_FAMILY_NAME + "_" + tenantId).setKey(queryPlanIdentifier).setRange("", "", false, 1000).setColumnNames(revision);

        QueryResult<ColumnSlice<String, byte[]>> r = q.execute();

        cs = r.get();
        return cs.getColumns().get(cs.getColumns().size() - 1).getValue();
