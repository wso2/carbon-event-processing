...
public final class RDBMSEventAdapterType extends AbstractOutputEventAdapter {
    ...
    private ConcurrentHashMap<Integer, ConcurrentHashMap<OutputAdaptorConfiguration, ExecutionInfo>> initialConfiguration;
    private Map<String, Map<String, String>> dbTypeMappings;


    private RDBMSEventAdapterType() {
        this.initialConfiguration = new ConcurrentHashMap<Integer, ConcurrentHashMap<OutputAdaptorConfiguration, ExecutionInfo>>();
    }
    ...
    protected void publish(
            Object message, OutputAdaptorConfiguration outputAdaptorConfiguration,
            int tenantId) {
        ...
        try {
            if (message instanceof Map) {
                ...
                tableName = outputAdaptorConfiguration.getEndpointAdaptorProperties().get(RDBMSEventAdaptorConstants.ADAPTOR_GENERIC_RDBMS_TABLE_NAME);
                String executionMode = outputAdaptorConfiguration.getEndpointAdaptorProperties().get(RDBMSEventAdaptorConstants.ADAPTOR_GENERIC_RDBMS_EXECUTION_MODE);
                String updateColumnKeys = outputAdaptorConfiguration.getEndpointAdaptorProperties().get(RDBMSEventAdaptorConstants.ADAPTOR_GENERIC_RDBMS_UPDATE_KEYS);

                ConcurrentHashMap<OutputAdaptorConfiguration, ExecutionInfo> outputAdapterConfigurationMap = initialConfiguration.get(tenantId);

                if (outputAdapterConfigurationMap != null) {
                    executionInfo = outputAdapterConfigurationMap.get(outputAdaptorConfiguration);
                } else {
                    outputAdapterConfigurationMap = new ConcurrentHashMap<OutputAdaptorConfiguration, ExecutionInfo>();
                ...
                if (executionInfo == null) {
                    ...
                    outputAdapterConfigurationMap.put(outputAdaptorConfiguration, executionInfo);
                    initializeDatabaseExecutionInfo(tableName, executionMode, updateColumnKeys, message, outputAdaptorConfiguration, executionInfo);
                }
            ...
            log.error("Error while initializing connection for datasource " + outputAdaptorConfiguration.getEndpointAdaptorProperties().get(RDBMSEventAdaptorConstants.ADAPTOR_GENERIC_RDBMS_DATASOURCE_NAME)
    ...
    private void initializeDatabaseExecutionInfo(String tableName, String executionMode, String updateColumnKeys, Object message,
                                                 OutputAdaptorConfiguration outputAdaptorConfiguration, ExecutionInfo executionInfo) throws RDBMSConnectionException {
        ...
        try {
            CarbonDataSource carbonDataSource = EventAdaptorValueHolder.getDataSourceService().getDataSource(outputAdaptorConfiguration.getEndpointAdaptorProperties().get(RDBMSEventAdaptorConstants.ADAPTOR_GENERIC_RDBMS_DATASOURCE_NAME));
            ...
            log.error("There is no any data-source found called : " + outputAdaptorConfiguration.getEndpointAdaptorProperties().get(RDBMSEventAdaptorConstants.ADAPTOR_GENERIC_RDBMS_DATASOURCE_NAME), e);
    ...
    @Override
    public void testConnection(OutputAdaptorConfiguration outputAdaptorConfiguration, int tenantId) {

        try {
            DataSource dataSource = null;
            CarbonDataSource carbonDataSource = EventAdaptorValueHolder.getDataSourceService().
                    getDataSource(outputAdaptorConfiguration.getEndpointAdaptorProperties().
    ...
    @Override
    public void removeConnectionInfo(OutputAdaptorConfiguration outputAdaptorConfiguration, int tenantId) {

        initialConfiguration.get(tenantId).remove(outputAdaptorConfiguration);
