...
import org.wso2.carbon.event.publisher.core.exception.EventFormatterStreamValidationException;
import org.wso2.carbon.event.publisher.core.exception.EventFormatterValidationException;
...
public class FormatterConfigurationBuilder {
    ...
    private static boolean validateEventAdaptor(String eventAdaptorName,
        ...
        if (eventAdaptorInfoList == null || eventAdaptorInfoList.size() == 0) {
            throw new EventFormatterValidationException("Event adaptor with name: " + eventAdaptorName + " and type: " + eventAdaptorType + " does not exist", eventAdaptorName);
        }
    ...
    public static EventFormatterConfiguration getEventFormatterConfiguration(
        ...
        if (!validateEventAdaptor(toEventAdaptorName, toEventAdaptorType, tenantId)) {
            throw new EventFormatterValidationException("Event Adaptor with name: " + toEventAdaptorName + " and type: " + toEventAdaptorType + " does not exist", toEventAdaptorName);
        }

        if (!validateStreamDetails(fromStreamName, fromStreamVersion, tenantId)) {
            throw new EventFormatterStreamValidationException("Stream " + fromStreamName + ":" + fromStreamVersion + " does not exist", fromStreamName + ":" + fromStreamVersion);
        }
