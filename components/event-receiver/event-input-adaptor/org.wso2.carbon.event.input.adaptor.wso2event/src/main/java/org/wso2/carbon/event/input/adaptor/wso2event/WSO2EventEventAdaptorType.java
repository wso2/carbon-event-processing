...
public final class WSO2EventEventAdaptorType extends AbstractInputEventAdaptor {
    ...
    private ConcurrentHashMap<Integer, ConcurrentHashMap<InputEventAdaptorConfiguration, ConcurrentHashMap<String, EventAdaptorConf>>> inputEventAdaptorListenerMap =
            new ConcurrentHashMap<Integer, ConcurrentHashMap<InputEventAdaptorConfiguration, ConcurrentHashMap<String, EventAdaptorConf>>>();
    private ConcurrentHashMap<Integer, ConcurrentHashMap<InputEventAdaptorConfiguration, StreamDefinition>> inputStreamDefinitionMap =
            new ConcurrentHashMap<Integer, ConcurrentHashMap<InputEventAdaptorConfiguration, StreamDefinition>>();
    ...
    public String subscribe(
        ...
        if (!tenantSpecificAdaptorListenerMap.keySet().contains(inputEventAdaptorConfiguration)) {
            ...
            tenantSpecificAdaptorListenerMap.get(inputEventAdaptorConfiguration).put(subscriptionId, eventAdaptorConf);
            ConcurrentHashMap<InputEventAdaptorConfiguration, StreamDefinition> tenantSpecificInputStreamDefinitionMap = inputStreamDefinitionMap.get(tenantId);
            if (tenantSpecificInputStreamDefinitionMap != null) {
                StreamDefinition streamDefinition = tenantSpecificInputStreamDefinitionMap.get(inputEventAdaptorConfiguration);
    ...
    public void unsubscribe(
        ...
        ConcurrentHashMap<InputEventAdaptorConfiguration, ConcurrentHashMap<String, EventAdaptorConf>> tenantSpecificAdaptorListenerMap = inputEventAdaptorListenerMap.get(PrivilegedCarbonContext.getThreadLocalCarbonContext().getTenantId(true));
        if (tenantSpecificAdaptorListenerMap != null) {
            Map<String, EventAdaptorConf> map = tenantSpecificAdaptorListenerMap.get(inputEventAdaptorConfiguration);
            if (map != null) {
    ...
    private class AgentTransportCallback implements AgentCallback {
        ...
        public void removeStream(StreamDefinition streamDefinition, int tenantId) {
            ConcurrentHashMap<InputEventAdaptorConfiguration, StreamDefinition> tenantSpecificSteamDefinitionMap = inputStreamDefinitionMap.get(tenantId);
            if (tenantSpecificSteamDefinitionMap != null) {
                tenantSpecificSteamDefinitionMap.remove(createTopic(streamDefinition));
            }

            ConcurrentHashMap<InputEventAdaptorConfiguration, ConcurrentHashMap<String, EventAdaptorConf>> tenantSpecificAdaptorListenerMap = WSO2EventEventAdaptorType.this.inputEventAdaptorListenerMap.get(tenantId);
            if (tenantSpecificAdaptorListenerMap != null) {
