...
public class EventStreamAdminService extends AbstractAdmin {
	...
	public void addEventStreamDefinition(String eventStreamName,
			...
			if ((eventStreamVersion != null) && (!eventStreamVersion.isEmpty())) {
				...
				try {
					StreamDefinition streamDefinition = new StreamDefinition(
							eventStreamName, eventStreamVersion);
							...
							streamDefinition
									.addMetaData(
											eventStreamAttributeDto
													.getAttributeName(),
											EventAttributeTypeConstants.STRING_ATTRIBUTE_TYPE_MAP
													.get(eventStreamAttributeDto
															.getAttributeType()));
						}
					}
					...
					EventStreamService eventStreamService = EventStreamAdminServiceValueHolder
							.getEventStreamService();
					eventStreamService.addEventStreamDefinition(
							streamDefinition, tenantId);

				} catch (MalformedStreamDefinitionException e) {
					throw new AxisFault("Not a valid stream definition "
							+ e.getMessage());
    ...
    public void addEventStreamDefinitionAsString(String streamStringDefinition)
        ...
        try {
            streamDefinition = EventDefinitionConverterUtils
                    .convertFromJson(streamStringDefinition);
            ...
            log.info("inside MalformedStreamDefinitionException");
            throw new AxisFault(e.getMessage(),e);
        } catch (EventStreamConfigurationException e) {
            throw new AxisFault(e.getMessage(),e);
        }
	...
	public void editEventStreamDefinition(String oldStreamId, String eventStreamName,
			String eventStreamVersion,
			EventStreamAttributeDto[] metaEventStreamAttributeDtos,
			EventStreamAttributeDto[] correlationEventStreamAttributeDtos,
			EventStreamAttributeDto[] payloadEventStreamAttributeDtos,
		...
		if ((eventStreamName != null) && (!eventStreamName.isEmpty())) {
			if ((eventStreamVersion != null) && (!eventStreamVersion.isEmpty())) {
				...
				try {
					...
					EventStreamService eventStreamService = EventStreamAdminServiceValueHolder
							.getEventStreamService();
					eventStreamService.removeEventStreamDefinition(
							oldStreamProperties[0], oldStreamProperties[1],
							tenantId);
					eventStreamService.addEventStreamDefinition(
							streamDefinition, tenantId);
	...
	public void removeEventStreamDefinition(String eventStreamName,
			String eventStreamVersion) throws AxisFault {
		...
		if ((eventStreamName != null) && (!eventStreamName.isEmpty())) {
			if ((eventStreamVersion != null) && (!eventStreamVersion.isEmpty())) {
				...
				try {
					eventStreamService.removeEventStreamDefinition(
							eventStreamName, eventStreamVersion, tenantId);
				} catch (EventStreamConfigurationException e) {
	...
	public EventStreamInfoDto[] getAllEventStreamDefinitionDto() throws AxisFault {
		...
		int tenantId = PrivilegedCarbonContext.getThreadLocalCarbonContext()
				.getTenantId();
		try {
			Collection<StreamDefinition> eventStreamDefinitionList = eventStreamService
					.getAllStreamDefinitions(tenantId);
			if (eventStreamDefinitionList != null) {
				EventStreamInfoDto[] eventStreamInfoDtos = new EventStreamInfoDto[eventStreamDefinitionList
						.size()];
				int index = 0;
				for (StreamDefinition streamDefinition : eventStreamDefinitionList) {
					eventStreamInfoDtos[index] = new EventStreamInfoDto();
					eventStreamInfoDtos[index].setStreamName(streamDefinition
							.getName());
					eventStreamInfoDtos[index]
							.setStreamVersion(streamDefinition.getVersion());
					eventStreamInfoDtos[index]
							.setStreamDefinition(streamDefinition.toString());
					eventStreamInfoDtos[index]
							.setStreamDescription(streamDefinition
									.getDescription());
					index++;
				}
	...
	public String[] getStreamNames() throws AxisFault {
		...
		if (eventStreamService != null) {
			int tenantId = PrivilegedCarbonContext
					.getThreadLocalCarbonContext().getTenantId();
			try {
				List<String> streamIdList = eventStreamService
						.getStreamIds(tenantId);
				...
				throw new AxisFault(
						"Error while retrieving stream names from store : "
								+ e.getMessage(), e);
			}
		}
		return new String[0];
	...
	public String[] getStreamDetailsForStreamId(String streamId)
		...
		if (eventStreamService != null) {
			...
			StreamDefinition streamDefinition = null;
			try {
				streamDefinition = eventStreamService.getStreamDefinition(
						streamId, tenantId);
				String[] streamDetails = new String[2];
				streamDetails[0] = streamDefinition.toString();
				streamDetails[1] = generateSampleEvent(streamId,
						EventAttributeTypeConstants.xmlEvent);
				return streamDetails;
			} catch (EventStreamConfigurationException e) {
				throw new AxisFault(
						"Error while retrieving stream definition from store : "
								+ e.getMessage(), e);
			}
		}
		return new String[0];
	...
	public String generateSampleEvent(String streamId, String eventType)
		...
		if (eventStreamService != null) {
			int tenantId = PrivilegedCarbonContext
					.getThreadLocalCarbonContext().getTenantId();
			try {
				return eventStreamService.generateSampleEvent(streamId,
						eventType, tenantId);
			} catch (EventStreamConfigurationException e) {
	...
	public String getStreamDefinitionAsString(String streamId) throws AxisFault {
		...
		if (eventStreamService != null) {
			...
			if (streamDefinition.getCorrelationData() != null) {
				...
				for (Attribute attribute : streamDefinition
						.getCorrelationData()) {
					if (appendComma) {
						definitionString = definitionString + ", ";
					}
	...
	public EventStreamDefinitionDto getStreamDefinitionDto(String streamId)
		...
		if (eventStreamService != null) {
			...
			try {
				...
				dto.setMetaData(convertAttributeList(streamDefinition
						.getMetaData()));
				dto.setCorrelationData(convertAttributeList(streamDefinition
						.getCorrelationData()));
				dto.setPayloadData(convertAttributeList(streamDefinition
						.getPayloadData()));
				return dto;
    ...
    public String convertEventStreamDefinitionDtoToString(
        ...
        try {
            streamDefinition = new StreamDefinition(
                    eventStreamDefinitionDto.getName(),
                    eventStreamDefinitionDto.getVersion());
        } catch (MalformedStreamDefinitionException e) {
            log.error(e.getMessage(), e);
            throw new AxisFault("Error while converting Dto");
        }

        streamDefinition.setDescription(eventStreamDefinitionDto
                .getDescription());
        streamDefinition.setNickName(eventStreamDefinitionDto.getNickName());
        streamDefinition
                .setCorrelationData(convertEventStreamAttributeDto(eventStreamDefinitionDto
                        .getCorrelationData()));
        streamDefinition
                .setMetaData(convertEventStreamAttributeDto(eventStreamDefinitionDto
                        .getMetaData()));
        streamDefinition
                .setPayloadData(convertEventStreamAttributeDto(eventStreamDefinitionDto
                        .getPayloadData()));
    ...
    public EventStreamDefinitionDto convertStringToEventStreamDefinitionDto(
            String streamStringDefinition) throws AxisFault{
        StreamDefinition streamDefinition = null;
        try {
            streamDefinition = EventDefinitionConverterUtils
                    .convertFromJson(streamStringDefinition);
        ...
        eventStreamDefinitionDto
                .setMetaData(convertAttributeList(streamDefinition
                        .getMetaData()));
        eventStreamDefinitionDto
                .setCorrelationData(convertAttributeList(streamDefinition
                        .getCorrelationData()));
        eventStreamDefinitionDto
                .setPayloadData(convertAttributeList(streamDefinition
                        .getPayloadData()));
	...
	private List<Attribute> convertEventStreamAttributeDto(
			EventStreamAttributeDto[] eventStreamAttributeDto) throws AxisFault {
		...
		if (eventStreamAttributeDto != null) {
			for (int i = 0; i < eventStreamAttributeDto.length; i++) {
				...
				Attribute attribute = new Attribute(
						eventStreamAttributeDto[i].getAttributeName(),
						attributeType);
