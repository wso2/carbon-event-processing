...
import org.wso2.carbon.event.publisher.core.config.EventFormatter;
...
import org.wso2.carbon.event.publisher.core.internal.util.EventFormatterUtil;
import org.wso2.carbon.event.publisher.core.internal.util.FormatterConfigurationBuilder;
import org.wso2.carbon.event.publisher.core.internal.util.helper.EventFormatterConfigurationFilesystemInvoker;
import org.wso2.carbon.event.publisher.core.internal.util.helper.EventFormatterConfigurationHelper;
...
public class CarbonEventFormatterService
        implements EventFormatterService {
    ...
    public void deployEventFormatterConfiguration(
        ...
        try {
            omElement = AXIOMUtil.stringToOM(eventFormatterConfigXml);
        } catch (XMLStreamException e) {
            throw new EventFormatterConfigurationException("Error in parsing XML configuration of event formatter.");
        }
        EventFormatterConfigurationHelper.validateEventFormatterConfiguration(omElement);
        String eventFormatterName = EventFormatterConfigurationHelper.getEventFormatterName(omElement);
        if (EventFormatterConfigurationHelper.getOutputMappingType(omElement) != null) {
                ...
                if ((eventFormatterConfigurationFile.getFileName().equals(fileName))) {
                    if (eventFormatterConfigurationFile.getStatus().equals(EventFormatterConfigurationFile.Status.DEPLOYED)) {
                        ...
                        if (tenantSpecificEventFormatterConfigurationMap.get(tenantId) != null) {
                            ...
                            EventFormatterServiceValueHolder.getEventStreamService().unsubscribe(eventFormatter, tenantId);
    ...
    public void activateInactiveEventFormatterConfigurationForStream(int tenantId, String streamId)
        ...
        if (eventFormatterConfigurationFileMap != null && eventFormatterConfigurationFileMap.size() > 0) {
            ...
            if (eventFormatterConfigurationFileList != null) {
                for (EventFormatterConfigurationFile eventFormatterConfigurationFile : eventFormatterConfigurationFileList) {
                    if ((eventFormatterConfigurationFile.getStatus().equals(EventFormatterConfigurationFile.Status.WAITING_FOR_STREAM_DEPENDENCY)) && eventFormatterConfigurationFile.getDependency().equalsIgnoreCase(streamId)) {
                        fileList.add(eventFormatterConfigurationFile);
                    }
                }
            }
        ...
        if (tenantSpecificEventFormatterConfigurationMap != null && tenantSpecificEventFormatterConfigurationMap.size() > 0) {
            ...
            if (eventFormatterMap != null) {
                for (EventFormatter eventFormatter : eventFormatterMap.values()) {
                    ...
                    if (eventAdaptorName.equals(dependency)) {
                        ...
                        if (eventFormatterConfigurationFile != null) {
                            fileList.add(eventFormatterConfigurationFile);
                            eventAdaptorService.removeConnectionInfo(eventFormatter.getEventFormatterConfiguration().getToPropertyConfiguration().getOutputEventAdaptorMessageConfiguration(), eventFormatter.getOutputEventAdaptorConfiguration(), tenantId);
                        }
                    }
                }
            }
