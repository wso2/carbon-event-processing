...
public class JMSEventAdaptorServiceHolder {
    ...
    private static final org.apache.commons.logging.Log log = LogFactory.getLog(JMSEventAdaptorServiceHolder.class);

    public static void registerConfigurationContextService(
            ConfigurationContextService configurationContextService) {
        JMSEventAdaptorServiceHolder.configurationContextService = configurationContextService;
        new Thread(new Runnable() {
            @Override
            public void run() {
                try {
                    log.info("JMS input event adaptor waiting for dependent configurations to load");
                    Thread.sleep(JMSEventAdaptorConstants.AXIS_TIME_INTERVAL_IN_MILLISECONDS * 4);
