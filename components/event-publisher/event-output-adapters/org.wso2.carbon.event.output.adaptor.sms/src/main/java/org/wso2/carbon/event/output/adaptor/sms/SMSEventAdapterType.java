...
public final class SMSEventAdapterType extends AbstractOutputEventAdapter {

    private static final Log log = LogFactory.getLog(SMSEventAdapterType.class);

    private static SMSEventAdapterType SMSEventAdaptor = new SMSEventAdapterType();
    private ConcurrentHashMap<OutputAdaptorConfiguration, List<String>> smsSenderConfigurationMap = new ConcurrentHashMap<OutputAdaptorConfiguration, List<String>>();
    private ResourceBundle resourceBundle;

    private SMSEventAdapterType() {
     ...
     */
    public static SMSEventAdapterType getInstance() {
    ...
    public void publish(
        ...
        List<String> smsList = smsSenderConfigurationMap.get(outputAdaptorConfiguration.getEndpointAdaptorProperties().get(SMSEventAdaptorConstants.ADAPTOR_MESSAGE_SMS_NO));
        if (smsList == null) {
            smsList = new ArrayList<String>();
            smsList.add(outputAdaptorConfiguration.getEndpointAdaptorProperties().get(SMSEventAdaptorConstants.ADAPTOR_MESSAGE_SMS_NO));
            smsSenderConfigurationMap.putIfAbsent(outputAdaptorConfiguration, smsList);
        }
        ...
        if (smsNOs != null) {
            for (String smsNo : smsNOs) {
                ...
                try {
                    ...
                    smsSenderConfigurationMap.remove(outputAdaptorConfiguration.getEndpointAdaptorProperties().get(SMSEventAdaptorConstants.ADAPTOR_MESSAGE_SMS_NO));
    ...
    public void removeConnectionInfo(
            OutputAdaptorConfiguration outputAdaptorConfiguration, int tenantId) {
        smsSenderConfigurationMap.remove(outputAdaptorConfiguration);
    }
