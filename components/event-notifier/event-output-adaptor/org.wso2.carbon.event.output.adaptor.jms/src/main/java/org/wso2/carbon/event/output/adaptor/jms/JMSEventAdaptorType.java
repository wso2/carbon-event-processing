...
public final class JMSEventAdaptorType extends AbstractOutputEventAdaptor {
    ...
    public void publish(
        ...
        ConcurrentHashMap<String, PublisherDetails> topicEventSender = publisherMap.get(endpointAdaptorConfiguration.getEndpointAdaptorName());
        if (null == topicEventSender) {
            topicEventSender = new ConcurrentHashMap<String, PublisherDetails>();
            if (null != publisherMap.putIfAbsent(endpointAdaptorConfiguration.getEndpointAdaptorName(), topicEventSender)) {
                topicEventSender = publisherMap.get(endpointAdaptorConfiguration.getEndpointAdaptorName());
            }
        }

        String topicName = endpointAdaptorConfiguration.getOutputAdaptorProperties().get(JMSEventAdaptorConstants.ADAPTOR_JMS_DESTINATION);
        ...
        try {
            if (null == publisherDetails) {
                publisherDetails = initPublisher(endpointAdaptorConfiguration, topicEventSender, topicName, messageConfig);
            }
            Message jmsMessage = publisherDetails.getJmsMessageSender().convertToJMSMessage(message, messageConfig);
            setJMSTransportHeaders(jmsMessage, endpointAdaptorConfiguration.getOutputAdaptorProperties().get(JMSEventAdaptorConstants.ADAPTOR_JMS_HEADER));
    ...
    private PublisherDetails initPublisher(
        ...
        adaptorProperties.putAll(endpointAdaptorConfiguration.getOutputAdaptorProperties());

        JMSConnectionFactory jmsConnectionFactory = new JMSConnectionFactory(adaptorProperties, endpointAdaptorConfiguration.getEndpointAdaptorName());
    ...
    public void removeConnectionInfo(
            EndpointAdaptorConfiguration endpointAdaptorConfiguration, int tenantId) {
        ConcurrentHashMap<String, PublisherDetails> topicEventSenderMap = publisherMap.get(endpointAdaptorConfiguration.getEndpointAdaptorName());
